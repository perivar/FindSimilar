...
namespace Mirage
{
	public class Mir
	{
		...
		/// <param name="databaseService">database</param>
				...
				Parallel.ForEach(filesRemaining, file =>
				                 {
				                 	...
				                 	foreach (string file in filesRemaining)
				                 	{
				                 		...
				                 		if ( (skipDurationAboveSeconds > 0 && duration > 0 && duration < skipDurationAboveSeconds)
				                 			...
				                 			if(!Analyzer.AnalyzeAndAdd(fileInfo, db, databaseService)) {
				...
				public static void Main(string[] args) {
					...
					if (scanPath != "") {
						if (IOUtils.IsDirectory(scanPath)) {
							...
							ScanDirectory(scanPath, db, databaseService, skipDurationAboveSeconds);
						} else {
