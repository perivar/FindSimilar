...
public class Program
{
	...
	public static void HashTester(string[] argv)
	{
		if(argv.Length == 1)
		{
			...
			Console.WriteLine(ImageHash.AverageHash(theImage).ToString("x16") + "\t" + argv[0]);
		}
		else if(argv.Length == 2)
		{
			...
			/*
			ulong hash1 = ImageHash.AverageHash(theImage);
			ulong hash2 = ImageHash.AverageHash(theOtherImage);
			
			Console.WriteLine(hash1.ToString("x16") + "\t" + argv[0]);
			Console.WriteLine(hash2.ToString("x16") + "\t" + argv[1]);
			Console.WriteLine("Similarity: " + ImageHash.Similarity(hash1, hash2) + "%");
			 */
